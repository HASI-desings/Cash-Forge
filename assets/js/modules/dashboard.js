...
function initDashboard() {
    // State.checkProtection() is run by auth.js globally, ensuring user is logged in.
    
    // 1. Get User Data and Update Static Elements
    const user = State.get('user');
    if (user) {
        // Update VIP Badge (Specific to Dashboard Header)
        const vipEl = document.getElementById('vip-level-display');
        if (vipEl) vipEl.innerText = `VIP ${user.vipLevel}`;
    ...
    // 2. Render Investment Plans using CONFIG data
    renderPlans();

    // 3. Start Promo Slider
    initSlider();

    // 4. Update the UI balance (Though bound globally in state.js, calling here ensures initial render)
    DB.updateUIBalance();
}
...
function renderPlans() {
    ...
    const plansToShow = CONFIG.PLANS.slice(0, 3); 

    plansToShow.forEach(plan => {
        const dailyIncomeFormatted = CONFIG.formatCurrency(plan.daily);
